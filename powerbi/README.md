...
The project includes six reporting tables, five shared dimensions, eight validated one-to-many relationships, 17 DAX measures, and five populated report pages. The separate [`../snowflake/`](../snowflake/) folder retains the dimensional Snowflake deployment path for environments where credentials are available.
...
4. Select **Refresh** to load the six public synthetic marts and five conformed dimensions.
...
The demo intentionally uses aggregated marts rather than patient- or claim-level records. Shared Date, Provider, Specialty, Claim Status, and Condition dimensions provide safe filtering relationships without creating ambiguous fact-to-fact joins. This keeps the download small and makes the public Power BI review path independent of Snowflake credentials.
