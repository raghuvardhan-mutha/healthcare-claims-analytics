...
| UAT-04 | Open the `.pbip`, set Snowflake parameters, and refresh | Four model tables load and relationships remain active | Requires Power BI Desktop and Snowflake |
| UAT-05 | Compare Power BI Total Claims and Total Paid with validation log | Values reconcile for the same filters and build seed | Pending UAT-04 |
| UAT-06 | Filter year, claim type, specialty, and provider | All intended visuals update consistently | Pending UAT-04 |
| UAT-07 | Select a payment-integrity pattern | Provider/claim details narrow without language asserting fraud | Pending UAT-04 |
